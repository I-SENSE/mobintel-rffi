...
%   Copyright 2019-2023 The MathWorks, Inc.
    ...
    methods
        ...
        %                   * Incomplete packet
        %                   * Unsupported format - Format other than EHT,
        %                     HE, VHT, HT-MF, HT-GF or Non-HT detected
        ...
        %                   * HE-SIG-B User Fail
        %                   * U-SIG FailCRC
        %                   * Invalid U-SIG contents - CRC passed but
        %                     contents invalid
        %                   * EHT-SIG Common Fail
        %                   * EHT-SIG User Fail
        ...
        function T = macSummary(obj,pktNum,verbose)
        %macSummary Display MAC summary of the selected packet
        %
        ...
        %                              column is only displayed for EHT and
        %                              HE packet format.
        %
        %   STAID/UserNumber           The STAID refer to the association
        %                              identifier (AID) of EHT and HE-MU
        %                              user. The STAID column is only
        %                              displayed for EHT and HE-MU packet
        %                              format. For VHT UserNumber
        %                              represents a user in a VHT MU
        %                              packet. UserNumber is an integer
        %                              between 1 and NumUsers, where
        %                              NumUsers is the number of users in
        %                              VHT MU packet. The UserNumber is
        %                              only displayed for an VHT MU packet
        %                              format.
        %
        ...
        %   selected packet in a table.
            ...
            if any(strcmp(x.Format,{'EHT-MU','HE-SU','HE-MU','HE-EXT-SU','VHT','HT-MF'})) && ~strcmp(x.MAC(1).DeagStatus,'N/A') % If deaggregation is N/A for the first users only possible in HT-MF
                ...
                ampduIndex = [];
                for u=1:numUsers
                    if any(strcmp(x.Format,{'EHT-MU','HE-SU','HE-MU','HE-EXT-SU'}))
                        cfgRx = x.PHYConfig(u);
                        STAID = cfgRx.STAID; % Only applicable for EHT MU and HE MU
                    end
            ...
            elseif any(strcmp(x.Format,{'EHT-MU','HE-MU','VHT'}))
        ...
        %   PKTNUM.
            ...
            hAreas = gobjects(1,6);
            figure;
            % plot(20*log10(abs(obj.rawWaveform+eps))+30); % The null subcarriers in the waveform have zero power. Add eps to avoid infinity resulting due to the log of zero. 
            plot(real(obj.rawWaveform));
            hold on;
            ...
            switch x.Format
                ...
                case {'EHT-MU'}
                    hAreas(5) = area([pktStart pktEnd],[max(ylim) max(ylim)],min(ylim),'FaceAlpha',0.1,'FaceColor','c');
                otherwise
                    hAreas(6) = area([pktStart pktEnd],[max(ylim) max(ylim)],min(ylim),'FaceAlpha',0.1,'FaceColor','k');
            end
            ...
            formatVec = ["Non-HT","VHT","HT-MF","HE","EHT",x.Format];
            ...
            figure;
            plot(freqAxis/1e6,psdAvg); % In MHz
        ...
        %   The table contains the following columns:
        %
        %   Field Name            Field names for an EHT-MU packet as
        %                         one of L-STF, L-LTF, L-SIG, RL-SIG,
        %                         U-SIG, EHT-SIG, EHT-STF, EHT-LTF, and
        %                         Data are displayed.
        %
        %                         Field names for an HE-MU packet as
        %                         one of L-STF, L-LTF, L-SIG, RL-SIG,
        %                         HE-SIG-A, HE-SIG-B, HE-STF, HE-LTF and
        %                         Data are displayed. The HE-SIG-B field is
        %                         hidden for HE-SU and HE-EXT-SU packet
        %                         formats.
        %
        %                         Field names for an VHT packet as one of
        %                         L-STF, L-LTF, L-SIG, VHT-SIG-A, VHT-STF,
        %                         VHT-LTF, VHT-SIG-B and Data are
        %                         displayed.
        %
        %                         Field names for an HT-MF packet as one of
        %                         L-STF, L-LTF, L-SIG, HT-SIG, HT-STF,
        %                         HT-LTF and Data are displayed.
        %
        %                         Field names for a Non-HT packet as one of
        ...
        %   summary of the selected packet in a table.
            ...
            elseif strcmp(x.Format,'HT-MF')
                ...
                if x.HTData.Processed % Update only for non NDP
                    ...
                    mcs = rem(cfgRx.MCS,8);
                    modType(7,:) = getHERateDependentParameters(mcs);
                end
            ...
            elseif strcmp(x.Format,'EHT-MU')
                ...
                if strcmp(x.Format,'HE-MU')
                    if x.HESIGB.User.Processed % If user field is successfully processed only then display the relevant information
        ...
        %   field summary of the selected packet in a table.
            ...
            elseif strcmp(x.Format,'EHT-MU')
                ...
                if cfgRx.PPDUType==wlan.type.EHTPPDUType.ndp
                    ...
                    if cfgRx.PPDUType==wlan.type.EHTPPDUType.dl_ofdma
                        if any(strcmp(cfgRx.ChannelBandwidth,{'CBW20','CBW40','CBW80'}))
                ...
                fprintf([pad(' ',offset),'<strong>Signaling Field Summary of Packet %d (%s)</strong>\n\n'],pktNum,x.Format);
            ...
            if any(strcmp(x.Format,{'HE-MU','HE-SU','HE-EXT-SU','EHT-MU'}))
        ...
        %   the selected packet in a table.
            ...
            if strcmp(x.Format,'EHT-MU')
                ...
                for ru=1:numRUs
                    ruName(ru) = string(['RU' num2str(ru)]);
                ...
                if numRUs==1 %#ok<ISCL> % EHT SU
                    ...
                    ruTable = table(ruName,strtrim(cell2mat(ruSizeTemp)),ruStartInd,ruEndInd,numUserPerRU,numSTSPerRU,ruPower);
                end
        ...
        %                           displaced for EHT and HE MU packet
        %                           format. User Number is only displayed
        %                           for an VHT multiuser packet format.
        %
        %   RU Number               Resource unit number. This column is
        %                           only displayed for EHT and HE MU packet
        %                           format.
        %
        ...
        %                           column is only displayed for EHT and HE
        %                           MU packet format.
        %
        ...
        %
        %   RMS EVM                RMS EVM in dB of the data field.
        %
        %   Max EVM                Max EVM in dB of the data field.
        %
            ...
            elseif strcmp(x.Format,"HE-MU")
                ...
                for ru=1:numRUs
                    ...
                    for u=1:numUserPerRU
                        ...
                        nsts = cfgRx.NumSpaceTimeStreams;
                        stdID = [stdID; repmat(cfgRx.STAID,nsts,1)]; %#ok<*AGROW>
                        evmRMS = [evmRMS; cfgPkt.HEData.User(userNumber).EVMRMS];
                        evmMax = [evmMax; cfgPkt.HEData.User(userNumber).EVMMax];
                        numSTS =  [numSTS; (1:nsts).'];
                    end
                end
        ...
        %spatial streams.
            ...
            plotFlag = ~any(strcmp(x.Format,{'Non-HT','HT-MF','HE-SU'})) && ~(any(strcmp(x.Format,{'VHT','EHT-MU'})) && numUsers==1);
            if plotFlag
        ...
        function plotSpectralFlatness(obj,chanbw,pktNum)
        %plotSpectralFlatness Plot spectral flatness of a selected packet.
            ...
            switch x.Format                
                ...
                case "EHT-MU"
                    pktFormat = "EHT";
                    if isscalar(x.EHTPreamble.RU) && x.PHYConfig(1).PuncturedChannelFieldValue==0
                        chanEst = x.EHTPreamble.RU(1).ChanEst;
                    else % OFDMA allocations is not supported
                        return;
                    end
                otherwise % "HE-SU","HE-MU", or "HE-EXT-SU"
                    pktFormat = "HE";
                    if isscalar(x.HEPreamble.RU)
            ...
            [pass,deviation,testSC] = wlanSpectralFlatness(chanEst,pktFormat,chanbw);
            fprintf(' \n');
            if pass
            ...
            title(ax,sprintf('Spectral flatness (packet %d)',pktNum));
            ...
            legend(ax,legendEntries,'location','south');
        end
...
% getEVMPerSubcarrier Get EVM per subcarrier
    ...
    switch field
        case 'EHT'
            ofdmInfo = wlanEHTOFDMInfo('EHT-Data',cfgRx);
        case 'HE'
...
%lsigRate LSIG field rate table
    ...
    out = ['0x' dec2hex(bit2int(rate,height(rate)))];
end
...
function limitPlot(dBr,indRange,testSC)
