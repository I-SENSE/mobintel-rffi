function [ofdmDemod,cpe,peg,pilotGain] = helperTrackingOFDMDemodulate(rxData,chanEst,numOFDMSym,cfg,cfgTrack,varargin)
%helperTrackingOFDMDemodulate OFDM demodulation with pilot tracking
%
%   [DATASYM,CPE,PEG,PILOTGAIN] = helperTrackingOFDMDemodulate(RXDATA,CHANEST,
%   NUMOFDMSYM,CFGSU,CFGTRACK) performs OFDM demodulation of data symbols
%   with optional pilot tracking.
%
...
%
%   CFGSU is a format configuration object of type <a href="matlab:help('wlanHESUConfig')">wlanHESUConfig</a>,  
%   <a href="matlab:help('wlanHERecoveryConfig')">wlanHERecoveryConfig</a>, or <a href="matlab:help('wlanEHTRecoveryConfig')">wlanEHTRecoveryConfig</a>.
%
%   CFGTRACK is a pilot tracking configuration structure.
%
%   [...] = helperTrackingOFDMDemodulate(RXDATA,CHANEST,NUMOFDMSYM,CFGMU,
%   CFGTRACK,RUNUMBER) performs OFDM demodulation of data symbols with
%   optional pilot tracking for a multi-user configuration. As a multi-user
%   configuration is provided and resource unit number is required.
%
...
if strcmp(cfgTrack.PilotTracking,'Joint')
    ...
    [ofdmDemod,peg,cpe,pilotGain] = demodulateWithPhaseTracking(rxData,chanEst,numOFDMSym,cfg,cfgTrack,varargin{:});
else
    % OFDM demodulate only when no joint tracking
    isEHT = isa(cfg,'wlanEHTRecoveryConfig') || isa(cfg,'wlanEHTMUConfig');
    if isa(cfg,'wlanHERecoveryConfig')
        ofdmDemod = wlanHEDemodulate(rxData,'HE-Data',cfg.ChannelBandwidth,cfg.GuardInterval, ...
            [cfg.RUSize cfg.RUIndex],'OFDMSymbolOffset',cfgTrack.OFDMSymbolOffset);
    elseif isEHT
        ofdmDemod = wlanEHTDemodulate(rxData,'EHT-Data',cfg,varargin{:});
    else
        ofdmDemod = wlanHEDemodulate(rxData,'HE-Data',cfg,varargin{:},'OFDMSymbolOffset',cfgTrack.OFDMSymbolOffset);
    end
    ...
    if strcmp(cfgTrack.PilotTracking,'CPE')
        ...
        if isEHT
            [ofdmDemod,cpe] = wlanEHTTrackPilotError(ofdmDemod,chanEst,cfg,'EHT-Data',varargin{:});
        else
            [ofdmDemod,cpe] = wlanHETrackPilotError(ofdmDemod,chanEst,cfg,'HE-Data',varargin{:});
        end
...
elseif isEHT
    ...
    if isEHTRecovery
        ...
        numSIGSyms1 = ehtSIGInfo.NumSIGSymbols; % Number of EHT-SIG symbols
        ...
        if ~isempty(varargin)
            % For OFDMA syntax
            ruNumber = varargin{1};
            ruSize = sum(ruSize{ruNumber}); % Could be MRU
            numSpaceTimeStreamsPerRU = numSpaceTimeStreamsPerRU(ruNumber);
        else
            ruSize = ruSize{ruNumber};
        end
    end
    ...
    if isa(cfg,'wlanHEMUConfig')
        ...
        numSIGSyms1 = sigbInfo.NumSymbols; % Number of HE-SIG-B symbols
...
if isEHT
    refPilots = wlan.internal.ehtPilots(ruSize,nsts,n,z);
else
