...
if isSUTx       
    ...
    channelCodingBit = bit2int(VHTSIGABits(2, 3), 1, false);
    ...
    for userIdx = 1:maxUsers
        ...
        if userIdx == 1
            % Read B2 for 1 user           
            channelCodingBits(1) = bit2int(VHTSIGABits(2, 3), 1, false);
        else
            % Read B4:B6 for 2:4 users
            channelCodingBits(userIdx) = bit2int(VHTSIGABits(2, 3+userIdx), 1, false);
        end        
    end
