...
class blk(gr.sync_block):
    def __init__(self, save_enabled=0, directory="/root/rec", usrp_1="24-5_A.bin", usrp_2="24-5_B.bin", usrp_3="24-6_A.bin", usrp_4="24-6_B.bin"):
        ...
        self.file_names = [usrp_1, usrp_2, usrp_3, usrp_4]
        self.file_paths = [os.path.join(self.directory, usrp) for usrp in self.file_names]
        self.file_handles = [None] * 4
        self.file_check_counter = 0

        self.sample_counter = [0] * 4 # we'll keep track of how many samples are we saving, to stop when >= MAX_SAMP_PER_RECORD
    ...
    def work(self, input_items, output_items):
        if self.save_enabled:
            if self.file_check_counter == 0:  # Adjust the frequency of file checks as needed
                print("Check")
