...
% Process HE-SIG-A and HE-SIG-B
    ...
    [hesigaDemod,cpe] = wlanHETrackPilotError(sigaDemod,preHEChanEst,cfgRx,'HE-SIG-A');

    % Equalize data carrying subcarriers, merging 20 MHz subchannels
    preheInfo = wlanHEOFDMInfo('HE-SIG-A',chanBW);
    [eqSIGASymComb,csi] = wlanHEEqualize(hesigaDemod(preheInfo.DataIndices,:,:), ...
                                         preHEChanEst(preheInfo.DataIndices,:,:), ...
                                         NEstNonHT,cfgRx,'HE-SIG-A');
    % Equalize without combining for EVM measurement
    eqSIGASym = ofdmEqualize(hesigaDemod(preheInfo.DataIndices,:,:),preHEChanEst(preheInfo.DataIndices,:,:),NEstNonHT);
    ...
    if strcmp(cfgRx.PacketFormat,'HE-MU')
        ...
        if ~cfgRx.SIGBCompression
            ...
            demodCommonSym = wlanHETrackPilotError(demodCommonSym,preHEChanEst(preheInfo.PilotIndices,:,:),cfgRx,'HE-SIG-B');

            % Extract data symbols
            demodCommonData = demodCommonSym(preheInfo.DataIndices,:,:);

            % Perform equalization
            [eqCommonSymComb,csiData] = wlanHEEqualize(demodCommonData,preHEChanEst(preheInfo.DataIndices,:,:),NEstNonHT,cfgRx,'HE-SIG-B'); %  With combining
            eqCommonSym = ofdmEqualize(demodCommonData,preHEChanEst(preheInfo.DataIndices,:,:),NEstNonHT); % Without combining
        ...
        % Estimate and correct common phase error
        demodUserFieldData = wlanHETrackPilotError(demodUserFieldData,preHEChanEst(preheInfo.PilotIndices,:,:),cfgRx,'HE-SIG-B');

        % Extract data symbols
        demodUserData = demodUserFieldData(preheInfo.DataIndices,:,:);

        % Perform equalization
        [eqUserSymComb,csi] = wlanHEEqualize(demodUserData,preHEChanEst(preheInfo.DataIndices,:,:),NEstNonHT,cfgRx,'HE-SIG-B'); % With combining
        eqUserSym = ofdmEqualize(demodUserData,preHEChanEst(preheInfo.DataIndices,:,:),NEstNonHT); % Without combining
...
% Process HE-Data field for a user
    ...
    res.NoiseEst = wlanHEDataNoiseEstimate(demodPilotSym,pilotEst,user);
    ...
    [res.EQDataSym,csiData] = wlanHEEqualize(demodSym(heOFDMInfo.DataIndices,:,:),chanEst(heOFDMInfo.DataIndices,:,:),nEstEQ,user,'HE-Data');
    % Equalize pilots separately as do not go through STBC combining
    res.EQPilotSym = ofdmEqualize(demodSym(heOFDMInfo.PilotIndices,:,:),chanEst(heOFDMInfo.PilotIndices,:,:),nEstEQ);

    % Demap and decode bits
    rxPSDU = wlanHEDataBitRecover(res.EQDataSym,res.NoiseEst,csiData,user,'LDPCDecodingMethod',cfgDataRec.LDPCDecodingMethod);
    res.rxPSDU = rxPSDU;
